...
```bash
# 单个订阅
http://your-domain:PORT/sub/{token}?target=clash
...
| 服务 | 默认端口 | 说明 |
|------|----------|------|
| 前端 (Nginx) | 8080 | 用户访问的 Web 界面 |
| 后端 (FastAPI) | 8081 | API 服务端口 |
| 数据库 (PostgreSQL) | 5432 | 数据库端口（仅内部） |
...
```bash
# 获取订阅列表
curl http://localhost:8081/api/subscriptions \
...
# 刷新订阅
curl -X POST http://localhost:8081/api/subscriptions/1/refresh \
  -H "Authorization: Bearer <token>"

# 导入节点
curl -X POST http://localhost:8081/api/nodes/import \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"uris":"vmess://xxx\nvless://yyy"}'
```
...
```bash
# 获取所有节点
curl http://localhost:8081/api/nodes/all \
  -H "Authorization: Bearer <token>"

# 测试节点延迟
curl -X POST http://localhost:8081/api/nodes/speedtest-all \
  -H "Authorization: Bearer <token>"
```
...
```bash
# 获取订阅内容
curl "http://localhost:8081/sub/{token}?target=clash"

# 按分组导出
curl "http://localhost:8081/sub/{token}/export/group?target=clash&group_by=region&group_value=HK"
...
```bash
# 构建后端镜像 (amd64)
VERSION=1.0.2 COMMIT=$(git rev-parse --short HEAD) \
  docker buildx build --platform linux/amd64 \
  -t subforge-backend:latest \
  -f backend-python/Dockerfile backend-python/ --load

# 构建前端镜像 (amd64)
cd frontend
docker buildx build --platform linux/amd64 \
  -t subforge-frontend:latest . --load

# 保存镜像
docker save subforge-backend:latest | gzip > images/subforge-backend.tar.gz
docker save subforge-frontend:latest | gzip > images/subforge-frontend.tar.gz
```
